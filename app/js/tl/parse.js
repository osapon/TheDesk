...
function parse(obj, mix, acct_id, tlid, popup, mutefilter, type) {
	...
	Object.keys(obj).forEach(function(key) {
		...
		if (content) {
				...
				if (tickerdata) {
					...
					for (var i = 0; i < tickerdata.length; i++) {
						...
						if (value.domain == thisdomain) {
							var tickerdom = '<div style="user-select:none;cursor:default;background:linear-gradient(90deg, ' + value.bg + ", transparent 96%) !important; color:" + value.text + ';width:100%; height:0.9rem; font-size:0.8rem;" class="tickers"><img draggable="false" src="' + value.image + '" style="height:100%;" onerror="this.src=\'../../img/loading.svg\'"><span style="position:relative; top:-0.2rem;"> ' + escapeHTML(value.name) + "</span></div>";
							break;
						}
					}
				}
			...
			templete = templete + `
			<div id="pub_${toot.id}" class="cvo ${boostback} ${fav_app} ${rt_app} ${pin_app} ${hasmedia} ${animecss}" toot-id="${id}" unique-id="${uniqueid}" data-medias="${media_ids} " unixtime="${date(obj[key].created_at, "unix")}" ${if_notf} onmouseover="mov('${toot.id}','${tlid}','mv')" onclick="mov('${toot.id}','${tlid}','cl')" onmouseout="resetmv('mv')">
				<div class="area-notice"><span class="gray sharesta">${notice}${home}</span></div>
				<div class="area-icon"><a onclick="udg('${toot.account.id}',${acct_id});" user="${toot.account.acct}" class="udg"><img draggable="false" src="${avatar}" width="40" class="prof-img" user="${toot.account.acct}" onerror="this.src='../../img/loading.svg'"></a>${noticeavatar}</div>
				<div class="area-display_name">
					<div class="flex-name"><span class="user">${dis_name}</span><span class="sml gray" style="overflow: hidden;white-space: nowrap;text-overflow: ellipsis; cursor:text;"> @${toot.account.acct}${locked}</span></div>
					<div class="flex-time">
						<span class="area-vis">${vis}</span>
						<span class="cbadge cbadge-hover pointer waves-effect" onclick="tootUriCopy('${toot.url}');" title="${date(toot.created_at, "absolute")}(${lang.lang_parse_clickcopyurl})"><i class="far fa-clock"></i>${date(toot.created_at, datetype)}</span>
					</div>
				</div>
				<div class="area-toot">${tickerdom}<span class="${api_spoil}" cw_text_${toot.id}"><span class="cw_text">${spoil}</span>${spoiler_show}</span><div class="toot ${spoiler}">${content}</div>${poll}${viewer}</div>
				<div class="area-additional"><span class="additional">${analyze}</span>${mentions}${tags}</div>
				<div class="area-actions ${mouseover}">
					...
					<div class="action ${disp["re"]} ${noauth}"><a onclick="re('${toot.id}','${to_mention}',${acct_id},'${visen}')" class="waves-effect waves-dark btn-flat actct rep-btn" data-men="${to_mention}" data-visen="${visen}" style="padding:0" title="${lang.lang_parse_replyto}"><i class="fas fa-share"></i><span class="rep_ct">${replyct}</a></span></a></div>
					<div class="action ${can_rt} ${disp["rt"]} ${noauth}"><a onclick="rt('${uniqueid}',${acct_id},'${tlid}')" class="waves-effect waves-dark btn-flat actct bt-btn" style="padding:0" title="${lang.lang_parse_bt}"><i class="fas fa-retweet ${if_rt} rt_${uniqueid}"></i><span class="rt_ct">${toot.reblogs_count}</span></a></div>
					<div class="action ${can_rt} ${disp["qt"]} ${noauth} ${qtClass}"><a onclick="qt('${toot.id}',${acct_id},'${toot.account.acct}','${toot.url}')" class="waves-effect waves-dark btn-flat actct" style="padding:0" title="${lang.lang_parse_quote}"><i class="text-darken-3 fas fa-quote-right"></i></a></div>
					<div class="action ${disp["fav"]} ${noauth}"><a onclick="fav('${uniqueid}',${acct_id},'${tlid}')" class="waves-effect waves-dark btn-flat actct fav-btn" style="padding:0" title="${lang.lang_parse_fav}"><i class="fas text-darken-3 fa-star${if_fav} fav_${uniqueid}"></i><span class="fav_ct">${toot.favourites_count}</span></a></div>
					<div class="${if_mine} action ${disp["del"]} ${noauth}"><a onclick="del('${toot.id}',${acct_id})" class="waves-effect waves-dark btn-flat actct" style="padding:0" title="${lang.lang_parse_del}"><i class="fas fa-trash"></i></a></div>
					<div class="${if_mine} action pin ${disp["pin"]} ${noauth}"><a onclick="pin('${toot.id}',${acct_id})" class="waves-effect waves-dark btn-flat actct" style="padding:0" title="${lang.lang_parse_pin}"><i class="fas fa-map-pin pin_${toot.id} ${if_pin}"></i></a></div>
					<div class="${if_mine} action ${disp["red"]} ${noauth}"><a onclick="redraft('${toot.id}',${acct_id})" class="waves-effect waves-dark btn-flat actct" style="padding:0" title="${lang.lang_parse_redraft}"><i class="material-icons">redo</i></a></div>
					${trans}
				</div>
				<div class="area-side ${mouseover}">
					<span class="cbadge viabadge waves-effect ${viashow} ${mine_via}" style="max-width:60px;" onclick="client('${$.strip_tags(via)}')" title="via ${$.strip_tags(via)}">${via}</span>
					...
					<div class="action ${noauth}"><a onclick="details('${toot.id}',${acct_id},'${tlid}','normal')" class="waves-effect waves-dark btn-flat details ${dmHide}" style="padding:0"><i class="text-darken-3 material-icons">more_vert</i></a></div>
				</div>
			</div>
			</div></div>`;
		}
